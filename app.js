(() => {
  ...
  analyzeBtn.addEventListener('click', async () => {
    ...
    results.hidden = true; summary.textContent = ''; metrics.textContent = ''; findings.textContent = ''; hooks.textContent = '';
    ...
    addMetric('مدت ویدیو', duration == null ? '—' : formatDuration(duration));
    addMetric('قاب', vertical === null ? '—' : (vertical ? 'عمودی' : `${width}×${height}`));
    addMetric('کپشن', `${capLen} کاراکتر`);
    addMetric('CTA', action ? 'دارد' : 'ندارد');
    ...
    if (duration !== null) {
      if (duration < 5) addFinding('ریسک طول خیلی کوتاه', 'نسخه‌ای حدود 7 تا 12 ثانیه را هم تست کن، اگر پیام محتوا اجازه می‌دهد.', 'warn');
      else if (duration > 45) addFinding('ریسک طول بالا', 'یک نسخه کوتاه‌تر از همان ایده بساز تا مشخص شود افت توجه از طول ویدیو می‌آید یا نه.', 'warn');
      else addFinding('طول در محدوده قابل‌تست', 'طول به‌تنهایی هشدار جدی ایجاد نمی‌کند؛ آزمایش را روی شروع و پیام اصلی متمرکز کن.', 'good');
    }
    if (vertical === false) addFinding('قاب غیربهینه برای Reel', 'نسخه 9:16 را تست کن تا محتوای اصلی فضای عمودی بیشتری بگیرد.', 'warn');
    if (!cap) addFinding('کپشن خالی است', 'یک کپشن کوتاه و مشخص اضافه کن و نتیجه را با نسخه بدون کپشن مقایسه کن.', 'warn');
    else if (capLen < 20) addFinding('کپشن بسیار کوتاه است', 'یک توضیح یا زمینه کوتاه اضافه کن که ارزش محتوا را روشن کند؛ سپس با نسخه فعلی مقایسه کن.', 'warn');
    else addFinding('کپشن قابل استفاده است', 'در آزمایش بعدی فقط یک متغیر کپشن را تغییر بده تا اثر آن قابل‌تشخیص بماند.', 'good');
    if (!action) addFinding('CTA مشخص نیست', 'اگر هدف تعامل، فالوور یا فروش است، یک دعوت به اقدام مشخص و متناسب با همان هدف تست کن.', 'warn');
    else addFinding('CTA ثبت شده', `CTA فعلی «${action}» است؛ آن را ثابت نگه دار و بیشتر روی هوک ابتدای ویدیو آزمایش کن.`, 'good');
  ...
  resetBtn.addEventListener('click', () => { videoInput.value = ''; selectedFile = null; videoMeta = null; fileName.textContent = 'ویدیو روی سرور آپلود نمی‌شود؛ تحلیل در مرورگر انجام می‌شود.'; urlInput.value = ''; caption.value = ''; cta.value = ''; results.hidden = true; setStatus(''); window.scrollTo({ top: 0, behavior: 'smooth' }); });
})();